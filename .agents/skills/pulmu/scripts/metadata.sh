...
case "$COMMAND" in
  finalize)
    TYPE=""; FORGE=""; RISK=""; AREAS=""; PATTERN=""; SECURITY=""; COMPAT=""
    while [[ $# -gt 0 ]]; do
      case "$1" in
    ...
    if [[ -z "$existing_status" ]]; then
      ...
      pulmu_metadata_write task_type "$TYPE"; pulmu_metadata_write base_branch "$legacy_base"; pulmu_metadata_write branch "$branch"; pulmu_metadata_write slug "${branch##*/}"
    ...
    printf 'PULMU_METADATA_STATUS=final\nPULMU_TYPE=%s\nPULMU_FORGE=%s\nPULMU_RISK=%s\nPULMU_AREAS=%s\nPULMU_PATTERN=%s\n' "$TYPE" "$FORGE" "$RISK" "$AREAS" "$PATTERN"
    ;;
  hone)
    RESULT=""
    while [[ $# -gt 0 ]]; do case "$1" in --result) RESULT="${2:-}"; shift 2 ;; *) pulmu_die "unknown metadata hone option: $1" ;; esac; done
    ...
    ;;
  delivery)
    TITLE=""; SUMMARY=""; RISK_REASON=""; declare -a CHANGES=() FOCUS=()
    while [[ $# -gt 0 ]]; do
      case "$1" in
        --title) TITLE="${2:-}"; shift 2 ;; --summary) SUMMARY="${2:-}"; shift 2 ;; --change) CHANGES+=("${2:-}"); shift 2 ;;
        --review-focus) FOCUS+=("${2:-}"); shift 2 ;; --risk-reason) RISK_REASON="${2:-}"; shift 2 ;; *) pulmu_die "unknown metadata delivery option: $1" ;;
      esac
    done
